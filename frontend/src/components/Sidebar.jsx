...
import { BellIcon, HomeIcon, ShipWheelIcon, User } from 'lucide-react';
    ...
    <aside className='w-64 bg-base-200 border-r border-base-300 hidden lg:flex flex-col h-screen sticky top-0'>
      ...
      <nav className='flex-1 p-4 space-y-1'>
        ...
        <Link to='/notifications' className={`btn btn-ghost justify-start w-full gap-3 px-3 normal-case ${currentPath === '/notifications' ? 'btn-active' : ''}`}>
          <BellIcon className='size-05 text-base-content opacity-70' />
      ...
      <div className='p-4 border-t border-base-300 mt-auto'>
        <div className='flex items-center gap-3'>
          ...
          <div className='flex-1'>
            ...
            <p className='text-xs text-success flex items-center gap-1'><span className='size-2 rounded-full bg-success inline-block' />Online</p>
          </div>
        </div>
      </div>
    </aside>
